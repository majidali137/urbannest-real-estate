...
const completeProfile = async (req, res) => {
  const { username, email, fullName, phone, city, requestSellerSession } =
    req.body;
  ...
  try {
    ...
    if (!user) {
      return res.json({
        error: "No user found with this email, Try logging in again",
      });
    }
    ...
    if (requestSellerSession) {
      ...
      res
        .status(201)
        .json({ success: "Profile updated, You are now a seller!" });
    } else {
...
const toggleSelling = async (req, res) => {
  ...
  try {
    ...
    if (role.name === "seller") {
      const newRole = await RoleModel.findOneAndUpdate(
        { _id: role._id },
        { $set: { name: "buyer", permissions: ["read"] } }
      );
      user = { ...user, roles: newRole };
